...
const versionPath = path.resolve(__dirname, "../backend/worker/src/version.json");
...
async function bumpBackendVersion() {
  let current = { version: "0.0.0", deployedAt: null };
  try {
    const raw = await readFile(versionPath, "utf8");
    current = JSON.parse(raw);
  } catch {
    current = { version: "0.0.0", deployedAt: null };
  }
  ...
  console.log(`Backend version bumped to v${next.version}`);
}
